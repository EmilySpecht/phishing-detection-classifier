...
class PhishingTrainer:
    ...
    def load_datasets(self) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
        ...
        return email_df, phish_df, malicious_df
    ...
    #   email_scores = np.array(
    #       email_model.predict_proba(X_train_email),
    #       dtype=float,
    #   )
    ...
    def filter_legitimate_urls(
        self,
        df: pd.DataFrame,
    ) -> pd.DataFrame:
        legitimate_df = (
            df[
                df["type"].str.lower() != "phishing"
            ]
    ...
    def train_url(self, phish_df: pd.DataFrame, malicious_df: pd.DataFrame):
        legitimate_df = self.filter_legitimate_urls(malicious_df)
        ...
        # majestic_df = majestic_df.rename(columns={
        #   "Domain": "URL"
        # })
            
        # majestic_df = majestic_df.sample(
        #     n=len(phish_df),
        #     random_state=42
        # )
        ...
        # phish_df = phish_df[["URL", "Label"]]
        # majestic_df = majestic_df[["URL", "Label"]]

        # url_df = pd.concat(
        #     [phish_df, majestic_df],
        #     ignore_index=True
        # )
    ...
    def train_all(self, email_df: pd.DataFrame, phish_df: pd.DataFrame, malicious_df: pd.DataFrame) -> dict[str, Any]:
        self.config.models_dir.mkdir(parents=True, exist_ok=True)

        email_metrics = self.train_email(email_df)
        url_metrics = self.train_url(phish_df, malicious_df)
